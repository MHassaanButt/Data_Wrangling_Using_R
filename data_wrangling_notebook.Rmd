...
# 1. Online hospitality company Airbnb
...
```{r}
### select specfic columns as per instructions in assignement and save in new dataframe
...
```{r}
### Checking columns datatype as it's required
...
### From above cell we can confirm that all columns are with correct datatypes except price it is char because of $ sign, I will change it latter doing section 1.4

```{r}
# Now printing the summary of filtered dataset
summary(df)
```
...
```{r}
# printing list of property type
unique(w_df$property_type)
```
```{r}
...
### Above is list of top 10 hosts names due to space issue we are unable to see their names.
...
# Add a month column:
#w_df$property_type <- property_type.abb[as.numeric(format(w_df$property_type, "%m"))]
#w_df$property_type <- factor(w_df$property_type, levels = property_type)
...
ggplot(data = w_df,
  ...
  labs(title = cl_site)
```
### Room types along with prices. boxes shows that on x-axis we have room types and on y-axis we have prices.
...
### we found above that none is found in column 1 and column 2 have its values so we want to fix it
...
```{r}
#my_data <- my_data %>% separate(Count/Percentage, c('Count', 'Percentage'))
...
### the datatype of columns are changed now
...
# df_count %>%
#    group_by(Count) %>%      # group your data based on the variable Rating
#    arrange(desc(regions)) %>%     # order in descending order the variable One_Year_Return
#    slice(1)
print(df_count)
```
